return {
    ...
    opts = {
        -- 自動讀取會話（啟動時）
        autoread = true, -- 改為 false，啟動時顯示首頁而非自動載入會話

        -- 自動保存會話（退出時）
        autowrite = true, -- 保持開啟，退出時自動保存
        ...
        hooks = {
            pre = {
                write = function()
                    ...
                    -- 關閉所有特殊類型的 buffer（避免 session 載入錯誤）
                    for _, buf in ipairs(vim.api.nvim_list_bufs()) do
                        if vim.api.nvim_buf_is_valid(buf) then
                            local buftype = vim.bo[buf].buftype
                            local filetype = vim.bo[buf].filetype
                            -- 刪除特殊 buffer
                            if buftype ~= "" or filetype == "lazy" or filetype == "mason" or filetype == "NvimTree" or filetype == "aerial" then
